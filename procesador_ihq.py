...
de los PDFs de IHQ, garantizando precisión al no     # 2) Fallback: tabla "Estudios solicitados …  Organo"
    if not organo_final:
    ...
    # 3) NUEVO: Buscar órganos en contextos específicos de procedimientos
    if not organo_final:
        full_text_norm = _normalize_text(text)
        procedure_patterns = [
            # Nefrectomías
            r'NEFRECTOMIA\s+(?:RADICAL\s+)?(?:IZQUIERDA|DERECHA|BILATERAL)?',
            r'RINON\s+(?:IZQUIERDO|DERECHO)',
            
            # Ginecológicos
            r'HISTERECTOMIA\s+(?:TOTAL|RADICAL)?',
            r'ANEXECTOMIA\s+(?:BILATERAL|UNILATERAL)?',
            r'(?:UTERO|CERVIX|OVARIO|ENDOMETRIO)',
            
            # Respiratorios
            r'(?:LOBECTOMIA|NEUMECTOMIA|TORACOTOMIA)',
            r'(?:PULMON|PLEURA|BRONQUIO)\s*(?:IZQUIERDO|DERECHO)?',
            
            # Digestivos
            r'(?:GASTRECTOMIA|COLECTOMIA|HEPATECTOMIA)',
            r'(?:ESTOMAGO|COLON|HIGADO|PANCREAS)',
            
            # Neurológicos
            r'(?:CRANIOTOMIA|LAMINECTOMIA)',
            r'(?:CEREBRO|TEMPORAL|FRONTAL|PARIETAL|HIPOFISIS)',
            
            # Mama y tejidos blandos
            r'(?:MASTECTOMIA|CUADRANTECTOMIA)',
            r'(?:MAMA|TEJIDO\s+BLANDO)',
            
            # Urológicos
            r'(?:PROSTATECTOMIA|ORQUIECTOMIA)',
            r'(?:PROSTATA|TESTICULO|VEJIGA)'
        ]
        ...
        for pattern in procedure_patterns:
            ...
            if match:
                organo_final = match.group(0)
                break

    data['organo_final'] = organo_final if organo_final else 'No especificado'stantes genéricas.
"""
...
try:
    from procesador_mejorado import detect_malignancy_advanced, extract_organ_advanced
    PROCESADOR_MEJORADO_DISPONIBLE = True
except ImportError:
    PROCESADOR_MEJORADO_DISPONIBLE = False
    print("AVISO: Procesador mejorado no disponible, usando funciones básicas")
...
def _normalize_text(text: str) -> str:
    """Normaliza texto removiendo acentos y estandarizando."""
    if not text:
        return ""
...
def detect_malignancy_ihq(*texts: str) -> str:
    """MEJORADO: Usa detección contextual avanzada para malignidad."""
    text_to_check = " ".join(_normalize_text(t) for t in texts if t).upper()
    
    # 1. Buscar términos directos de malignidad
    for keyword in MALIGNIDAD_KEYWORDS_IHQ:
        if re.search(r'\b' + keyword + r'\b', text_to_check):
            return 'PRESENTE'
    
    # 2. Buscar patrones contextuales específicos de IHQ
    malignancy_patterns = [
        r'NEOPLASIA\s+(?:RENAL|PAPILAR|ENDOCRINA|ONCOCITICA)',
        r'TUMOR\s+(?:PRIMARIO|SOLIDO|EPITELIAL)',
        r'CARCINOMA\s+(?:ESCAMOCELULAR|UROTELIAL|BASOCELULAR)',
        r'ADENOCARCINOMA\s+(?:METASTASICO|INVASIVO|PRIMARIO)',
        r'HALLAZGOS\s+(?:COMPATIBLES\s+CON|SUGIEREN)\s+(?:MALIGNIDAD|NEOPLASIA)',
        r'LESION\s+(?:MALIGNA|ONCOLOGICA)',
        r'(?:GRADO|ESTADIO)\s+(?:III|IV|3|4)',
        r'INVASION\s+(?:LINFOVASCULAR|PERINEURAL|VASCULAR)'
    ]
    
    for pattern in malignancy_patterns:
...
def extract_organ_advanced(text: str, organo_base: str = '') -> str:
    """
    Extracción avanzada de órganos con patrones de procedimientos médicos.
    Args:
        text: Texto completo del informe
        organo_base: Órgano detectado por métodos básicos
    Returns:
        Nombre del órgano detectado o "No especificado"
    """
    if organo_base and organo_base.strip() and organo_base != "No especificado":
        return organo_base
    
    text_norm = _normalize_text(text)
    
    # 1. Patrones de procedimientos quirúrgicos que indican órganos
    procedimientos_organos = {
        r'NEFRECTOMIA\s+(?:RADICAL\s+)?(?:IZQUIERDA|DERECHA|BILATERAL)?': 'RIÑON',
        r'HISTERECTOMIA\s+(?:TOTAL|RADICAL)?': 'UTERO',
        r'PROSTATECTOMIA\s+(?:RADICAL)?': 'PROSTATA',
        r'COLECTOMIA\s+(?:PARCIAL|TOTAL)?': 'COLON',
        r'MASTECTOMIA\s+(?:RADICAL|PARCIAL)?': 'MAMA',
        r'CRANIOTOMIA|CRANEOTOMIA': 'CEREBRO',
        r'TIROIDECTOMIA\s+(?:TOTAL|PARCIAL)?': 'TIROIDES',
        r'LOBECTOMIA\s+(?:PULMONAR)?': 'PULMON',
        r'GASTRECTOMIA\s+(?:TOTAL|PARCIAL)?': 'ESTOMAGO',
        r'HEPATECTOMIA\s+(?:PARCIAL)?': 'HIGADO',
        r'PANCREATICODUODENECTOMIA': 'PANCREAS',
        r'ESPLENECTOMIA': 'BAZO',
        r'LAPAROTOMIA\s+EXPLORATORIA': 'ABDOMEN'
    }
    
    # 2. Patrones directos de órganos
    organos_directos = {
        r'RIÑON\s+(?:DERECHO|IZQUIERDO)': 'RIÑON',
        r'(?:UTERO|MATRIZ)\s+(?:Y\s+ANEXOS)?': 'UTERO',
        r'PROSTATA\s+(?:Y\s+VESICULAS)?': 'PROSTATA',
        r'(?:MAMA|SENO)\s+(?:DERECHA|IZQUIERDA)': 'MAMA',
        r'TIROIDES\s+(?:Y\s+PARATIROIDES)?': 'TIROIDES',
        r'(?:PULMON|LOBULO)\s+(?:DERECHO|IZQUIERDO)?': 'PULMON',
        r'(?:COLON|INTESTINO)\s+(?:SIGMOIDES?|ASCENDENTE|DESCENDENTE)?': 'COLON',
        r'(?:ESTOMAGO|GASTRICO)': 'ESTOMAGO',
        r'(?:HIGADO|HEPATICO)': 'HIGADO',
        r'PANCREAS\s+(?:Y\s+DUODENO)?': 'PANCREAS',
        r'(?:CEREBRO|CEREBRAL|ENCEFALICO)': 'CEREBRO',
        r'(?:BAZO|ESPLENICO)': 'BAZO'
    }
    
    # Buscar procedimientos primero
    for patron, organo in procedimientos_organos.items():
        if re.search(patron, text_norm, re.IGNORECASE):
            return organo
    
    # Buscar órganos directos
    for patron, organo in organos_directos.items():
        if re.search(patron, text_norm, re.IGNORECASE):
            return organo
    
    # 3. Patrones contextuales en descripciones
    context_patterns = [
        (r'BIOPSIA\s+DE\s+([A-Z\s]+?)(?:\s+CON|\s+DERECHO|\s+IZQUIERDO|$)', 1),
        (r'RESECCION\s+DE\s+([A-Z\s]+?)(?:\s+CON|\s+DERECHO|\s+IZQUIERDO|$)', 1),
        (r'TUMOR\s+DE\s+([A-Z\s]+?)(?:\s+CON|\s+DERECHO|\s+IZQUIERDO|$)', 1),
        (r'CARCINOMA\s+DE\s+([A-Z\s]+?)(?:\s+CON|\s+DERECHO|\s+IZQUIERDO|$)', 1),
    ]
    
    for patron, grupo in context_patterns:
        match = re.search(patron, text_norm, re.IGNORECASE)
        if match:
            candidato = match.group(grupo).strip()
            # Validar que sea un nombre de órgano válido
            if len(candidato) > 2 and candidato not in ['CON', 'PARA', 'QUE', 'DEL']:
                return candidato
...
def extract_multiline_name(text: str, pattern: str) -> str:
    """
    FUNCIÓN PERFECCIONADA: Extrae nombres que pueden estar en múltiples líneas.
    
    Maneja dos casos específicos encontrados en reportes IHQ del HUV:
    1. Nombres divididos: "MARCO ENRIQUE MIDEROS" (línea 1) + "RODRIGUEZ" (línea 2)
    2. Nombres completos: "GABRIEL ANGULO ANGULO" (una línea)
    
    Args:
        text: Texto del reporte donde buscar el nombre
        pattern: Patrón regex (se ignora, usa lógica especializada)
        
    Returns:
        str: Nombre completo limpio o cadena vacía si no se encuentra
    """
    
    # Patrón 1: Caso multilinea 
    # Captura desde "Nombre :" hasta "N. peticion", luego la siguiente línea hasta "N.Identificación"
    pattern_multiline = r'Nombre\s*:\s*(.*?)N\.\s*peticion.*?\n([^\n]+)\nN\.Identificación'
    match_multiline = re.search(pattern_multiline, text, re.IGNORECASE | re.DOTALL)
    
    if match_multiline:
        part1 = match_multiline.group(1).strip()
        part2 = match_multiline.group(2).strip()
        nombre_completo = f"{part1} {part2}".strip()
        nombre_completo = re.sub(r'\s+', ' ', nombre_completo)
        return nombre_completo
    
    # Patrón 2: Caso de una línea
    # Captura desde "Nombre :" hasta "N. peticion"
    pattern_single = r'Nombre\s*:\s*([^N]*?)N\.\s*peticion'
    match_single = re.search(pattern_single, text, re.IGNORECASE)
    
    if match_single:
        nombre = match_single.group(1).strip()
        nombre = re.sub(r'\s+', ' ', nombre)
        return nombre
    
    # Patrón 3: Fallback más amplio para casos especiales
    pattern_fallback = r'Nombre\s*:\s*([^\n]*?)(?=\s*N\.\s*peticion|\n)'
    match_fallback = re.search(pattern_fallback, text, re.IGNORECASE)
    
    if match_fallback:
        nombre = match_fallback.group(1).strip()
        nombre = re.sub(r'\s+', ' ', nombre)
        return nombre
...
def extract_ihq_data(text: str) -> dict:
    ...
    # 1. Extracción básica con patrones comunes - MANEJO ESPECIAL PARA NOMBRES MULTILINEA
    for key, pattern in PATTERNS_HUV.items():
        if key == 'nombre_completo':
            # Usar función especial para nombres que pueden estar en múltiples líneas
            data[key] = extract_multiline_name(text, pattern)
        else:
    ...
    # Usar extracción mejorada de órganos
    if PROCESADOR_MEJORADO_DISPONIBLE:
        data['organo_final'] = extract_organ_advanced(text, organo_final)
    else:
